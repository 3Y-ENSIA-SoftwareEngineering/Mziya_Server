...
const sendEmail = async (email, token, type) => {
  try {
    ...
    if (type === 'confirmation'){
      emailHtml = emailTemplate.replace('verification_link', `http://localhost:3000/api/auth/register/verify/${token}`);
    }else if (type === 'OTPverify'){
      emailHtml = emailTemplate.replace('{{otp}}', token);
    }else if (type === 'resetPassword'){
      emailHtml = emailTemplate.replace('reset_link', `http://agrisistatnce.netlify.app/auth/reset-password/${token}`);
    }else if (type === 'updateVerification'){
      emailHtml = emailTemplate.replace('verification_link', `http://agrisistance-server.onrender.com/api/profile/update-email/verify/${token}`);
    }
